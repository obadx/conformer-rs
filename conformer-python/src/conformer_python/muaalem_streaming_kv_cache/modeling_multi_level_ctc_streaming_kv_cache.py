# Wav2Vec2Bert streaming inference with per-layer KV cache + conv cache.
# Extends the HuggingFace Wav2Vec2Bert architecture with:
#   - Chunked streaming via cached key/value states
#   - FIFO eviction to bound cache size
#   - Optional sliding-window attention masking
#   - Cross-chunk convolutional state to prevent boundary artifacts
import math
import warnings
...
from .configuration_multi_level_ctc_streaming_kv_cache import (
...
class Wav2Vec2BertConvolutionModule(nn.Module):
    ...
    def forward(
        ...
        # Save the tail of the GLU output for the next chunk's depthwise conv context.
        # The depthwise conv uses causal left-padding; caching the previous chunk's tail
        # prevents information loss at chunk boundaries.
        cache_frames = self.depthwise_conv.kernel_size[0] - 1
        if conv_cache is not None:
            present_conv_cache = hidden_states[:, :, -cache_frames:].detach()
        else:
            present_conv_cache = None

        # Use cached context from the previous chunk instead of zero-padding.
        # This gives the depthwise conv proper left context across chunk boundaries.
        if conv_cache is not None:
            hidden_states = torch.cat([conv_cache, hidden_states], dim=-1)
        else:
            hidden_states = torch.nn.functional.pad(
                hidden_states, (cache_frames, 0)
            )
        ...
        hidden_states = self.dropout(hidden_states)
        hidden_states = hidden_states.transpose(1, 2)
        return hidden_states, present_conv_cache
...
class Wav2Vec2BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        sliding_window_left: Optional[int] = None,
        ...
        # Store current key/value for caching (before concatenation).
        # These will be returned to the caller for the next chunk.
        present_key_value = (key, value)

        # Concatenate with past KV cache if available (streaming inference).
        if past_key_value is not None:
            past_key, past_value = past_key_value

            # FIFO eviction: keep only the last max_context_frames.
            # When the combined cache + new chunk exceeds the limit,
            # drop the oldest frames from the cache. This bounds memory
            # for long audio streams.
            if max_context_frames is not None and past_key.size(-2) + key.size(-2) > max_context_frames:
                total_len = past_key.size(-2) + key.size(-2)
                frames_to_drop = total_len - max_context_frames
                past_key = past_key[..., frames_to_drop:, :]
                past_value = past_value[..., frames_to_drop:, :]

            key = torch.cat([past_key, key], dim=-2)
            value = torch.cat([past_value, value], dim=-2)

        # Extend attention_mask to cover cached positions.
        # The cached positions have no padding (they're from prior valid chunks),
        # so we add a zero-mask (attend to all) for the cached region.
        if attention_mask is not None and past_key_value is not None:
            cached_len = past_key_value[0].size(-2)
            cache_mask = torch.zeros(
                batch_size, 1, sequence_length, cached_len,
                device=hidden_states.device, dtype=attention_mask.dtype
            )
        ...
        if use_sliding_window and past_key_value is not None:
            cached_len = past_key_value[0].size(-2)
            total_key_len = key.size(-2)
            q_len = query.size(-2)

            if sliding_window_left is None:
                sliding_window_left = 25
            if sliding_window_right is None:
                sliding_window_right = 5

            # For each query, attend only to keys within [query - left, query + right].
            # Cached positions are all to the left of current chunk queries, so
            # sliding_window_left limits how far back the query can look into the cache.
            # sliding_window_right limits lookahead (mostly relevant within-chunk).
            query_positions = torch.arange(q_len, device=hidden_states.device).unsqueeze(1)
            key_positions = torch.arange(total_key_len, device=hidden_states.device).unsqueeze(0)
            offset = cached_len  # current chunk positions start after cached

            # gap = (cached_len + query_idx) - key_idx  (positive = key is in the past)
            gap = offset + query_positions - key_positions
            sliding_mask = (gap > sliding_window_left) | (gap < -sliding_window_right)
            sliding_mask = sliding_mask.unsqueeze(0).unsqueeze(0)
            scores = scores.masked_fill(sliding_mask, torch.finfo(scores.dtype).min)
        ...
        return hidden_states, probs, present_key_value
...
class Wav2Vec2BertEncoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        # 2. Self-Attention layer (with KV cache)
        hidden_states = self.self_attn_layer_norm(hidden_states)
        hidden_states, attn_weigts, present_key_value = self.self_attn(
        ...
        # 3. Convolutional Layer (with cross-chunk conv_cache)
        residual = hidden_states
        hidden_states, present_conv_cache = self.conv_module(
            hidden_states, attention_mask=conv_attention_mask, conv_cache=conv_cache
        )
        ...
        return hidden_states, attn_weigts, present_key_value, present_conv_cache
...
class Wav2Vec2BertEncoder(nn.Module):
    ...
    def forward(
        ...
        conv_caches: Optional[list] = None,
    ):
        all_hidden_states = () if output_hidden_states else None
        all_self_attentions = () if output_attentions else None
        present_key_values = [] if past_key_values is not None else None
        present_conv_caches = [] if conv_caches is not None else None
        ...
        for i, layer in enumerate(self.layers):
            ...
            if not skip_the_layer or synced_gpus:
                ...
                # pass KV cache for this layer if available
                if past_key_values is not None and i < len(past_key_values):
                    layer_kwargs["past_key_value"] = past_key_values[i]
                    layer_kwargs["max_context_frames"] = self.config.max_context_frames
                    layer_kwargs["use_sliding_window"] = self.config.use_sliding_window
                    layer_kwargs["sliding_window_left"] = self.config.sliding_window_left
                    layer_kwargs["sliding_window_right"] = self.config.sliding_window_right

                # pass conv cache for this layer if available
                if conv_caches is not None and i < len(conv_caches):
                    layer_kwargs["conv_cache"] = conv_caches[i]

                layer_outputs = layer(**layer_kwargs)
                hidden_states = layer_outputs[0]

                # collect present key/value for this layer
                if present_key_values is not None:
                    present_key_values.append(layer_outputs[2])

                # collect present conv cache for this layer
                if present_conv_caches is not None:
                    present_conv_caches.append(layer_outputs[3])
        ...
        if not return_dict:
            ...
            if present_key_values is not None:
                result = result + (present_key_values,)
            if present_conv_caches is not None:
                result = result + (present_conv_caches,)
            return result
        ...
        if present_conv_caches is not None:
            output["conv_caches"] = present_conv_caches
        return output
...
class Wav2Vec2BertPreTrainedModel(PreTrainedModel):
    config_class = Wav2Vec2BertForMultilevelCTCStreamingKVCacheConfig
...
class Wav2Vec2BertModel(Wav2Vec2BertPreTrainedModel):
    def __init__(self, config: Wav2Vec2BertForMultilevelCTCStreamingKVCacheConfig):
    ...
    def forward(
        ...
        past_key_values (`list` of `tuple(torch.FloatTensor)`, *optional*):
            List of `(key, value)` tuples from previous chunks, one per encoder layer.
        conv_caches (`list` of `torch.FloatTensor`, *optional*):
            List of conv state tensors from previous chunks, one per encoder layer.
            Each tensor has shape `(batch_size, hidden_size, kernel_size - 1)`.
        """
        output_attentions = (
            output_attentions
        ...
        # Force return_dict when using streaming state (KV cache / conv cache)
        # so we can extract them from the encoder output dict.
        encoder_return_dict = return_dict
        if past_key_values is not None or conv_caches is not None:
            encoder_return_dict = True
        ...
        if past_key_values is not None:
            present_key_values = encoder_outputs.get("past_key_values", None)

        present_conv_caches = None
        if conv_caches is not None:
            present_conv_caches = encoder_outputs.get("conv_caches", None)

        if not return_dict:
            result = (hidden_states, extract_features) + encoder_outputs[1:]
            if present_key_values is not None:
                result = result + (present_key_values,)
            if present_conv_caches is not None:
                result = result + (present_conv_caches,)
        ...
        output = Wav2Vec2BertBaseModelOutput(
            last_hidden_state=hidden_states,
...
# -----------------------------------------------
# Multi level CTC with streaming KV cache
# -----------------------------------------------


class Wav2Vec2BertForMultilevelCTCStreamingKVCache(Wav2Vec2BertPreTrainedModel):
    config_class = Wav2Vec2BertForMultilevelCTCStreamingKVCacheConfig
    ...
    def forward(
        ...
        past_key_values (`list` of `tuple(torch.FloatTensor)`, *optional*):
            List of `(key, value)` tuples from previous chunks, one per encoder layer.
            Shape of each tuple: `(batch, num_heads, seq_len, head_dim)`.
        conv_caches (`list` of `torch.FloatTensor`, *optional*):
            List of conv state tensors from previous chunks, one per encoder layer.
            Each tensor has shape `(batch_size, hidden_size, kernel_size - 1)`.
            Used to prevent information loss in the causal depthwise convolution
            at chunk boundaries. Returned alongside past_key_values for the next chunk.
        """
        if labels is not None:
        ...
        # Force return_dict=True when using streaming state for clean extraction
        has_streaming_state = past_key_values is not None or conv_caches is not None
        model_return_dict = return_dict
        if has_streaming_state:
            model_return_dict = True

        # During training, process full utterance (no chunking).
        # During inference, process with optional KV cache + conv cache.
        if self.training and not has_streaming_state:
            outputs = self.wav2vec2_bert(
                input_features,
                attention_mask=attention_mask,
                output_attentions=output_attentions,
            ...
            present_key_values = outputs.get("past_key_values", None)
            present_conv_caches = outputs.get("conv_caches", None)
        ...
        if present_conv_caches is not None:
            result["conv_caches"] = present_conv_caches
        return result


__all__ = ["Wav2Vec2BertForMultilevelCTCStreamingKVCache"]
